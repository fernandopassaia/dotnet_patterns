...
### Practical Part Images and Containers: -----------------------------------------------------------------------------------------------------------
...
docker run -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=yourStrong(!)Password' -e 'MSSQL_PID=Express' -p 1433:1433 -d mcr.microsoft.com/mssql/server:2019-CU5-ubuntu-16.04

**RabbitMQ**:
docker pull rabbitmq:3.8.5-management-alpine
...
**Important Note3**: To see the logs of a container (specially in case of errors) use "docker logs --follow c201".
...
mysql:
image: "mysql:5.7"
volumes: - studentsdata:/var/lib/mysql
networks: - backend
environment: - MYSQL_ROOT_PASSWORD=@1234Fd# - bind-address=0.0.0.0
...
mysql:
image: "mysql:5.7"
volumes: - studentsdata:/var/lib/mysql - ./MySqlInitScript:/docker-entrypoint-initdb.d
networks: - backend
environment: - MYSQL_ROOT_PASSWORD=@1234Fd# - bind-address=0.0.0.0
...
networks : - backend  
 ports : - 5000:80
environment : - DBHOST=mysql - ASPNETCORE_ENVIRONMENT=Development
depends_on : - mysql
...
**NOTE HERE ON 11062020**: I've archived this DockerFile and DockerCompose here because basically the .NET Core Docker Image don't work good with the SQL
Server Docker Image. Yeah, sounds crazy, but those images of MS is bugged. So I'll switch the DataBase to POSTGRESQL giving a better solution.
...
docker run -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=@1234Fd#' -e 'MSSQL_PID=Express' --name sqlserver -p 4002:1433 -d mcr.microsoft.com/mssql/server:2017-CU20-ubuntu-16.04

To connect to this instance of SQL Server you can use Azure Data Studio and: localhost,4002 as SERVER, sa as user, as @1234Fd# as password. Yeah it works! =)
...
networks:
testing_net:
ipv4_address: 198.162.1.10
ports : - 4001:4001
environment : - ASPNETCORE_ENVIRONMENT=Development
depends_on : - appfullstackdemo.database

    appfullstackdemo.sqltools:
        container_name: mssqlservertools
        image: mcr.microsoft.com/mssql-tools
        networks:
            testing_net:
                ipv4_address: 198.162.1.15

networks:
testing_net:
ipam:
driver: default
config: - subnet: 198.162.1.0/16
...
#this port is very important to expose, because it's the SQL Server
EXPOSE 4002
...
**Annottations:**
(1) Workdir /src (here I'm creating a folder called /src inside the container)
(2) ADD / ./ (here I'm copy ALL my actual folder (backEnd) to this folder src inside the container - so my entire solution will be copied to the container)

(3) Note: Because the DataBase will run in a separated container, we cannot run the MIGRATION process while building the APP. Yeah, but we found a way to solve it,
...
(4) I've changed the port of SQL from 1433 to 4002 on my local machine, basically because SQL Server is Running on My LocalHost (Windows / Linux), and yeah,
I'll have conflicts with it. So i have to change, so basically my container is running on 4002, and i need to connect it by this way.
...
docker network ls
docker network inspect 9acd (where this 4 digits is the start of backend_appfullstackdemo)
OR docker network inspect bridge

IF you want to inspect a IMAGE (if the image don't enter on the Network):
docker image inspect 8c2e (will inspect the IMAGE created by the build)
docker container inspect 8c2e (will inspect the CONTAINER running)

docker logs --follow 8c2e (will inspect the errors)

To Check IP Address:
docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 517b

Then i see that my connection string is 172.18.0.2, so i've changed it on my backend SOURCE on appsettings.json AND AppFullStackDemoContext.cs:
Data Source=172.18.0.2,4002;Initial Catalog=AppFullStackDemo;Persist Security Info=True;User ID=sa;Password=@1234Fd#
...
docker-compose down --remove-orphans (remove and force to remove the containers - better)
...
**How to DEBUG and see OUTPUTS in case of errors:**
docker-compose logs
docker logs -f mysql (or the 4 first digits of a container - docker logs -f 89c2)
docker run -it --entrypoint /bin/bash <image> (here if i put the IMAGE of my AspNet Core App, i can enter on the console)

To install **Tools** on the Containers and Use for Tests:
...
### New Version of DockerFile and DockerCompose with PostgreSQL for AppFullStackDemo:

First **Pull** the first two images:
docker pull postgres:10.13-alpine
docker pull adminer
docker pull mcr.microsoft.com/dotnet/core/sdk:3.1-bionic
docker pull mcr.microsoft.com/dotnet/core/aspnet:3.1-bionic
docker pull node:14.4.0-buster
...
To **ENTER** on a container Linux:
docker exec -it <container> bash
docker run -it --entrypoint /bin/bash <image>

If i want to **GENERATE THE IMAGE based on the Dockerfile**, just one image (not based on compose):
docker build . -t nomedaimagem:1.0

**Accesses to the Container**:
**To access the Pgadmin** use: localhost:5000
Create a Connection with: postgres_container (host name is always the container name, also in connectionstring inside .NetApp)
UserName: postgres - password: @1234Fd#

Sample of Connection String: "ServerLocalConnection": "server=postgres_container;port=5432;userid=postgres;database=AppFullStackDemo;password=@1234Fd#;"

**URL for the BackEnd**: http://localhost:4001/api/User/v1/test

**To ShutDown everything**:
docker-compose down --remove-orphans
docker images
docker image rmi 34cc (and the number of the respective images)

And here there's my **FINAL** Docker-compose and DockerFiles for the BackEnd and the Angular Projects:
...
**DockerFile for FrontEnd (DockerfileBackEnd)**:
...
**DockerFile for FrontEnd (DockerfileFrontEnd)**:
